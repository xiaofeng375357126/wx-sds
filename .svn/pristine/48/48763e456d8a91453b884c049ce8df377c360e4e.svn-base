...
Page({
  ...
  */
  formSubmit:function(e){
    ...
    if  (!value.id_card)  return  app.Tips({  title:  '请填写身份证号'  });
    if  (!/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(value.id_card))  return  app.Tips({  title:  '请输入正确的身份证号'  });
